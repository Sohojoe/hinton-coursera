...
load data.mat;
allData = [data.trainData data.testData data.validData];
numdims = size(data.trainData, 1);
D = numdims - 1;
M = floor(size(allData, 2) / N);
train_input = reshape(allData(1:D, 1:N * M), D, N, M);
train_target = reshape(allData(D + 1, 1:N * M), 1, N, M);
